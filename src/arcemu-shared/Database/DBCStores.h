/*
 * ArcNG MMORPG Server Software
 * Copyright (c) 2008-2012 ArcNG Server Team
...
ARCEMU_INLINE float GetScale(CreatureDisplayInfo *Scale)
{
	return Scale->Scale;
}

ARCEMU_INLINE float GetDBCCastTime(SpellRadius *radius)
{
...
ARCEMU_INLINE uint32 GetscalestatMultiplier(ScalingStatValuesEntry *ssvrow, uint32 flags)
{
	if(flags & 0x4001F)
	{
		if(flags & 0x00000001)
...
ARCEMU_INLINE uint32 GetscalestatArmorMod(ScalingStatValuesEntry *ssvrow, uint32 flags)
{
	if(flags & 0x00F001E0)
	{
		if(flags & 0x00000020)
...
ARCEMU_INLINE uint32 GetscalestatDPSMod(ScalingStatValuesEntry *ssvrow, uint32 flags)
{
	if(flags & 0x7E00)
	{
		if(flags & 0x00000200)
...
ARCEMU_INLINE uint32 GetscalestatSpellBonus(ScalingStatValuesEntry *ssvrow)
{
