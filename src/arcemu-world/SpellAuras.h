...
public:
	...
	ARCEMU_INLINE uint32 GetModCount() const { return m_modcount; }
	ARCEMU_INLINE Modifier *GetMod(uint32 x) { return &m_modList[x]; }

    ARCEMU_INLINE uint8 GetAuraSlot() const { return m_auraSlot; }
	void SetAuraSlot(uint8 slot) { m_auraSlot = slot; }
	ARCEMU_INLINE uint8 GetAuraFlags() const { return m_auraFlags; }
	void SetAuraFlags(uint8 flags) { m_auraFlags = flags; }
	void SetAuraFlag(uint16 flag) { if( ! (m_auraFlags & flag ) ) m_auraFlags |= flag; };
	ARCEMU_INLINE uint8 GetAuraLevel() const { return m_auraLevel; }
	...
	Unit* GetUnitCaster();
	ARCEMU_INLINE Object* GetCaster() {return TO_OBJECT(GetUnitCaster());}
	ARCEMU_INLINE uint64 GetCasterGUID(){return m_casterGuid;}

	ARCEMU_INLINE Object* GetTarget() {return ((m_target != NULL && m_target->IsInWorld()) ? TO_OBJECT(m_target): NULL);}
	ARCEMU_INLINE Unit* GetUnitTarget() {return TO_UNIT(GetTarget());}
	ARCEMU_INLINE uint64 GetTargetGUID(){return m_target->GetGUID();}
	...
	ARCEMU_INLINE uint32 GetMSExpiryTime()
	{
		if (GetDuration() == -1)
			return -1;
	...
	ARCEMU_INLINE bool HasMechanic(uint32 MechanicsType)
	{
		return Spell::HasMechanic(m_spellProto, MechanicsType);
	}
	ARCEMU_INLINE uint32 GetMechanic()
	{
		return Spell::GetMechanic(m_spellProto);
	}

	ARCEMU_INLINE uint32 GetMechanicOfEffect(uint32 i)
	{
...
private:
	...
	ARCEMU_INLINE void DurationPctMod(uint32 mechanic);
