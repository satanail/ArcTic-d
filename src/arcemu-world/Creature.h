...
public:
	...
	// Arena organizers
	ARCEMU_INLINE bool IsArenaOrganizer() { return ( HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PETITIONER ) && !HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TABARDDESIGNER ) ); }
	...
	ARCEMU_INLINE void SetSheatheForAttackType(uint8 type) { SetByte(UNIT_FIELD_BYTES_2, 0, type); }
	...
	Unit* GetSummonOwner();
	ARCEMU_INLINE int32 GetSummonSlot() { return SummonSlot; }
	ARCEMU_INLINE bool IsSummon() { return SummonOwner && SummonSlot > -1; }
	ARCEMU_INLINE void SetSummonOwnerSlot(uint64 ownerguid, int8 slot) { SummonOwner = ownerguid; SummonSlot = slot;}

	//Special summons, Totems
	ARCEMU_INLINE bool IsTotem() { return Totem;}
	ARCEMU_INLINE void SetTotem(bool totem) { Totem = totem;}
	...
	ARCEMU_INLINE CreatureProtoHeroic * GetProtoHeroic() { return proto_heroic; }
...
public:
	...
	ARCEMU_INLINE uint32 GetProtoItemDisplayId(uint8 i) { return GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i); }
