...
public:
	...
	ARCEMU_INLINE bool IsInCombat() { return m_lastStatus; }			// checks if we are in combat or not.
...
public:
	...
	bool ARCEMU_FASTCALL canReachWithAttack(Unit* pVictim);
	...
	ARCEMU_INLINE float GetSize() { return GetFloatValue(OBJECT_FIELD_SCALE_X) * GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS); }
	...
	// Stealth
	ARCEMU_INLINE int32 GetStealthLevel() { return (m_stealthLevel + (getLevel() * 5)); }
	...
	ARCEMU_INLINE bool InStealth() { return (m_stealth!=0 ? true : false); }
	float detectRange;
	...
	ARCEMU_INLINE void SetOnMeleeSpell(uint32 spell, uint8 cast_number ) { m_meleespell = spell; m_meleespell_cn = cast_number; }
	...
	ARCEMU_INLINE uint32 computeOverkill(uint32 damage)
	{
	...
	ARCEMU_INLINE int32 GetHealthPct() { return (int32)(GetUInt32Value(UNIT_FIELD_HEALTH) * 100 / std::max(1, (int32)GetUInt32Value(UNIT_FIELD_MAXHEALTH))); }
	...
	ARCEMU_INLINE int32 GetManaPct() { return (int32)(GetUInt32Value(UNIT_FIELD_POWER1) * 100 / std::max(1, (int32)GetUInt32Value(UNIT_FIELD_MAXPOWER1))); }
	...
	ARCEMU_INLINE int8 GetSeatID() { return m_inVehicleSeatId; }
	ARCEMU_INLINE Unit* GetVehicle(bool forcevehicle = false)
	{
		if(m_CurrentVehicle)
	...
	ARCEMU_INLINE uint32 GetPowerType(){ return (GetByte(UNIT_FIELD_BYTES_0,3));}
