...
#define BANNER "WoWArc MMORPG Server :: WorldServer r%u/%s-%s-%s\n"

static const char* default_config_file = "configs/WorldServer.conf";
...
bool Master::Run(int argc, char ** argv)
{
	...
	if(geteuid() == 0 || getegid() == 0)
		Log.LargeErrorMessage( LARGERRORMESSAGE_WARNING, "You are running Arc as root.", "This is not needed, and may be a possible security risk.", "It is advised to hit CTRL+C now and", "start as a non-privileged user.", NULL);
#endif
	...
	/* write pid file */
	FILE * fPid = fopen( "configs/WorldServer.pid", "w" );
	if( fPid )
	{
		uint32 pid;
#ifdef WIN32
	...
	// remove pid
	remove("configs/WorldServer.pid");
