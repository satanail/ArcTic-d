...
public:
	...
	ARCEMU_INLINE void SendPacket(StackPacket * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }

	void ARCEMU_FASTCALL OutPacket(uint16 opcode, size_t len, const void* data);
	OUTPACKET_RESULT ARCEMU_FASTCALL _OutPacket(uint16 opcode, size_t len, const void* data);
	...
	void ARCEMU_FASTCALL UpdateQueuePosition(uint32 Position);
...
public:
	...
	ARCEMU_INLINE void SendPacket(StackPacket * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBuffer*() : NULL)); }
	void ARCEMU_FASTCALL OutPacket(uint16 opcode, uint16 len, const void* data);
