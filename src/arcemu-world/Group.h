...
public:
	...
	ARCEMU_INLINE void SendPacketToAll(StackPacket *packet) { SendPacketToAllButOne(packet, NULL); }
	...
	ARCEMU_INLINE int8 GetDifficulty() { return m_difficulty; }
	ARCEMU_INLINE int8 GetRaidDifficulty() { return m_raiddifficulty; }
	ARCEMU_INLINE void SetDifficulty(uint8 diff) { m_difficulty = diff; }
	ARCEMU_INLINE void SetRaidDifficulty(uint8 diff) { m_raiddifficulty = diff; }

	uint8 m_difficulty;
	uint8 m_raiddifficulty;

	ARCEMU_INLINE void SetFlag(uint8 groupflag) { m_groupFlags |= groupflag; }
	ARCEMU_INLINE void RemoveFlag(uint8 groupflag) { m_groupFlags &= ~groupflag; }
	ARCEMU_INLINE bool HasFlag(uint8 groupflag) { return (m_groupFlags & groupflag) > 0 ? true : false; }
