...
public:
    ...
    ARCEMU_INLINE SpellEntry *GetSpellProto() { return m_spellInfo; }
	...
	ARCEMU_INLINE uint32 GetType() { return ( m_spellInfo->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }
	...
	ARCEMU_INLINE float GetDBCCastTime(uint32 i)
	{
	...
	ARCEMU_INLINE static bool HasMechanic(SpellEntry * sp, uint32 MechanicsType)
	{
	...
	ARCEMU_INLINE static uint32 GetMechanicOfEffect(SpellEntry * sp, uint32 i)
	{
	...
	ARCEMU_INLINE static bool EffectAffectsSpell(SpellEntry* spellEffect, uint32 effectNum, SpellEntry* testSpell)
	{
