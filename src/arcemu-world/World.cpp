...
bool World::SetInitialWorldSettings()
{
	...
	Log.Notice("WoWArc", "I'm starting up!");
...
{
	if(load) { Config.MainConfig.SetSource("configs/WorldServer.conf", true); }
	...
	SetMotd(Config.MainConfig.GetStringDefault("Server", "Motd", "").c_str());
	...
	// Конфиг кика игроков с большим пингом
	LatencyKickEnabled = Config.MainConfig.GetBoolDefault("Server", "LatencyKick", false); // Данной ф-ии может и нет на оффе, но она очень полезная
