...
ARCEMU_INLINE uint8 SchoolMask(uint8 school)
{
...
ARCEMU_INLINE bool IsCastedOnFriends(SpellEntry *sp)
{
...
ARCEMU_INLINE bool IsCastedOnEnemies(SpellEntry *sp)
{
	for( int frloop = 0; frloop < 3; frloop++ )
	{
		switch (sp->EffectImplicitTargetA[frloop])
...
ARCEMU_INLINE bool IsHealingSpell(SpellEntry *sp)
{
	for(uint32 i = 0; i < 3; ++i)
	{
		switch( sp->Effect[i] )
...
ARCEMU_INLINE bool IsFlyingSpell(SpellEntry *sp)
{
