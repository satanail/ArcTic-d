...
#define BUG_TRACKER "https://github.com/arcdev/arcdev/issues"
...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
	...
	if( class_ == CLASS_DEATHKNIGHT && (!HasFlag(ACCOUNT_FLAG_XPACK_02) || !CanCreateDeathKnight() ) )
	{
...
#ifdef WIN32
	// _player->BroadcastMessage("Core server: Arc %s r%u/%s-Windows-%s (www.github.com/arcdev)", MSG_COLOR_WHITE, BUILD_TAG, BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);
#else
	// _player->BroadcastMessage("Core server: Arc %s r%u/%s-%s (www.github.com/arcdev)", MSG_COLOR_WHITE, BUILD_TAG, BUILD_REVISION, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE);
#endif
	// Bugs
