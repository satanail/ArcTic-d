...
public:
    ...
    void OnCombatStop(Unit* mTarget)
    {
		...
		if (_unit->isAlive()) {
			...
			_unit->GetMapMgr()->GetInterface()->SpawnCreature(19514, fly[11].mX, fly[11].mY, fly[11].mZ, 0, true, false, 0, 0);
		}
...
public:
	...
	void AIUpdate()
	{
		...
		{
			_unit->SetUInt32Value(UNIT_FIELD_HEALTH, (uint32)(CurrentHP - PercMaxHP));
...
public:
	...
	KaelThasAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		for (int i = 1; i < 4; ++i)
		{
			AddWaypoint(CreateWaypoint(1, 0, Waypoints[i].mAddition, Waypoints[i]));
		}
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			SpawnCreature(Advisors[i].mAddition, Advisors[i].mX, Advisors[i].mY, Advisors[i].mZ, Advisors[i].mO);
		}
		for (int i = 0; i < 2; ++i)
		{
	...
	void CheckAdvisorState(Creature* pCreature)
	{
		...
		{
			Coords pCoords;
			pCoords.mX = pCreature->GetPositionX();
			pCoords.mY = pCreature->GetPositionY();
			pCoords.mZ = pCreature->GetPositionZ();
			pCoords.mO = pCreature->GetOrientation();
			pCoords.mAddition = pCreature->GetEntry();
	...
	void AIUpdate()
	{
		if (GetPhase() < 5)
		{
			...
			{
				Coords pCoords;
				pCoords.mX = Advisors[i].mX;
				pCoords.mY = Advisors[i].mY;
				pCoords.mZ = Advisors[i].mZ;
				pCoords.mO = Advisors[i].mO;
				pCoords.mAddition = Advisors[i].mAddition;
				mAdvCoords.push_back(pCoords);
		...
		if (GetPhase() == 5)
		{
			...
			else if (mEventTimer == -2)
			{
				for (int i = 0; i < 7; ++i)
				{
					SpawnCreature(KaelthasWeapons[i].mAddition, KaelthasWeapons[i].mX, KaelthasWeapons[i].mY, KaelthasWeapons[i].mZ, KaelthasWeapons[i].mO);
				}
			...
			else if (IsTimerFinished(mEventTimer))
			{
				for (int i = 0; i < 4; ++i)
				{
					...
					MoonScriptCreatureAI *pAI = SpawnCreature(mAdvCoords[i].mAddition, mAdvCoords[i].mX, mAdvCoords[i].mY, mAdvCoords[i].mZ, false);
					if (pAI != NULL)
					{
