				...
				if ( Channeler && !Channeler->isAlive() && Channeler->IsInWorld() )
				{
					...
					for (size_t x = 0; x < ChannelersTable.size(); x++)
					{
						...
						if (BuffedChanneler && BuffedChanneler != Channeler && BuffedChanneler->isAlive())
						{
							...
							Aura* aura = NULL;
							aura = new Aura(dbcSpell.LookupEntry(SOUL_TRANSFER), (uint32)-1, BuffedChanneler, BuffedChanneler);
							if (aura != NULL)
							{
...
public:
	...
	void OnActivate(Player*  pPlayer)
	{
		...
		// If we haven't "filled" * already we do that now
		if (!CubeTrigger)
		...
		// We save player data in NULL as well as his position for further use
	...
	void AIUpdate()
	{
		...
		if (Counter >= ACTIVE_CUBES_TO_BANISH && Magtheridon && Magtheridon->isAlive())
		{
			...
			Aura* aura = NULL;
			aura = new Aura(dbcSpell.LookupEntry(MIND_EXHAUSTION),(uint32)78000, Magtheridon, Channeler);
			if (aura != NULL)
			{
...
public:
    ...
    MagtheridonAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		Aura* aura = NULL;
		aura = new Aura(dbcSpell.LookupEntry(BANISHMENT), (uint32)-1, _unit, _unit);
		if (aura != NULL)
		{
			_unit->AddAura(aura);
		}
