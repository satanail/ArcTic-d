...
public:
	...
	void SwitchToHumanForm()
	{
		...
		_unit->SetWeaponDisplayId(0, _unit->GetProtoItemDisplayId(0));
		_unit->SetWeaponDisplayId(1, _unit->GetProtoItemDisplayId(1));
	}

	void SwitchToDemonForm()
	{
		_unit->SetUInt32Value(UNIT_FIELD_DISPLAYID, 20125);
		_unit->SetWeaponDisplayId(0, 0);
		_unit->SetWeaponDisplayId(1, 0);
	}
...
public:
	...
	ShadowofLeotherasAI(Creature* pCreature) : CreatureAIScript(pCreature)
	{
		...
		sEventMgr.AddEvent(TO_UNIT(_unit), &Unit::EventAllowCombat, true, EVENT_CREATURE_UPDATE, 7500, 0, 1);
...
public:
		...
		for(unordered_set<Object*>::iterator itr = _unit->GetInRangeSetBegin(); itr != _unit->GetInRangeSetEnd(); itr++) 
		{
			if((*itr)->GetTypeId() == TYPEID_UNIT)
			{
				...
				if(creature && creature->creature_info && 
                    ...
                    creature->creature_info->Id == CN_SHIELD_GENERATOR_CHANNEL)
					&& creature->isAlive())
	...
	void PhaseTwo()
	{
		...
		{
			uint32 pos = rand()%8;
			_unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_TAINTED_ELEMENTAL, ElementalSpawnPoints[pos].mX, ElementalSpawnPoints[pos].mY, ElementalSpawnPoints[pos].mZ, ElementalSpawnPoints[pos].mO, true, false, 0, 0);
