...
class AshtongueSpiritbinderAI : public MoonScriptCreatureAI
{
	...
	AshtongueSpiritbinderAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		if (_unit->GetMapMgr()->iInstanceMode != MODE_HEROIC_5MEN)	// Guessed
...
public:
	...
	{
		DealDamageToFriends( static_cast<uint32>(fAmount), _unit->GetEntry() );
	};
...
public:
	...
	void OnDamageTaken(Unit* mAttacker, float fAmount) 
	{
		pGethois->DealDamageToFriends( static_cast<uint32>(fAmount), _unit->GetEntry() );
	};
...
class EyeBeamTriggerAI : public MoonScriptCreatureAI
{
    ...
    void AIUpdate()
    {
		...
		{
			MoveTo(EyeBeamPaths[7 - mPosition].mX, EyeBeamPaths[7 - mPosition].mY, EyeBeamPaths[7 - mPosition].mZ, false);
		...
		if ((mIllidanAI != NULL && mScenePart <= 6) || mScenePart > 6)
		{
			...
			case 6:{
				...
				if (pAkama != NULL)
				{
					...
					Coords pCoords;
					pCoords.mX = IllidanX - X;
					pCoords.mY = IllidanY - Y;
					pCoords.mZ = IllidanZ;
					pCoords.mO = 0.0f;
		...
		else if (GetHealthPercent() <= 65 && !IsCasting())
		{
			...
			if (pAkama != NULL && pAkama->GetScript() != NULL)
			{
				...
				if (pAkamaAI->mScenePart <= 2 && pAkamaAI->GetCanEnterCombat())
				{
					...
					pAkamaAI->GetUnit()->GetAIInterface()->SetNextTarget(NULL);
	...
	void PhaseTwo()
	{
		if (mScenePart != 0)
		{
			...
			case 1:
				for (int i = 0; i < 2; ++i)
				{
					_unit->CastSpellAoF(UnitPos[i].mX, UnitPos[i].mY, UnitPos[i].mZ, dbcSpell.LookupEntry(ILLIDAN_THROW_GLAIVE1), false);
				}
				SetWieldWeapon(false);
				break;
			case 2:
			...
			if (mMiscEventPart != 0)
			{
				if (mMiscEventPart == 1)
				{
					...
					if (pTrigger != NULL && pTrigger->GetScript() != NULL)
					{
						...
						float Distance = pTrigger->CalcDistance(EyeBeamPaths[7 - FireWall].mX, EyeBeamPaths[7 - FireWall].mY, EyeBeamPaths[7 - FireWall].mZ);
