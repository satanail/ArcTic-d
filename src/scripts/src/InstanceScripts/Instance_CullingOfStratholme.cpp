...
public:
    ...
    MEATHOOK_AI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		ConstrictingChains->target = SPELL_TARGET_LOWEST_THREAT;
	...
	void CastScriptSpell( ScriptSpell* spell )
	{
		...
		case SPELL_TARGET_LOWEST_THREAT:
			{
				int32 lowest = 0;
				if( _unit->GetAIInterface()->getAITargetsCount() > 0 )
				{
					for(TargetMap::iterator itr = _unit->GetAIInterface()->GetAITargets()->begin(); itr != _unit->GetAIInterface()->GetAITargets()->end(); itr++)
					{
						...
						{
							lowest = ( itr->second + itr->first->GetThreatModifier() ); 
							spelltarget = itr->first;
						}
					}
				}
			}break;
...
public:
	...
	void OnDamageTaken(Unit* mAttacker, float fAmount)
	{
		...
		{
			_unit->Heal( _unit, 52723, float2int32(fAmount/2.0f) );
		}
		...
		{//lala
			...
			RemoveAIUpdateEvent();
			_unit->SendChatMessage( CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, "Your journey has just begun, young prince. Gather your forces, and meet me in the arcemu land of Northrend. It is there we shall settle the score between us. It is there that your true destiny will unfold.");
    ...
    void AIUpdate()
	{
		...
		{//this is ugly, better ideas?
			...
			for (int i = 0; i != _unit->GetMapMgr()->m_CreatureHighGuid; ++i)
			{
				...
				{
					entry = _unit->GetMapMgr()->m_CreatureStorage[i]->GetEntry();
