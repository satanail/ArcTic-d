...
class PortalCrystalDalaranTop : public GameObjectAIScript
{
public:
	PortalCrystalDalaranTop(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject* GO) { return new PortalCrystalDalaranTop(GO); }

	void OnActivate(Player* pPlayer)
	{
		pPlayer->SafeTeleport(571, 2817, 5731.38f, 1014.95f, 174.48f, 3.64765f);		 //Teleport to Violet Stand Crystal (191229)
	}
};
...
class PortalCrystalDalaranLower : public GameObjectAIScript
{
public:
	PortalCrystalDalaranLower(GameObject* goinstance) : GameObjectAIScript(goinstance) {}
	static GameObjectAIScript *Create(GameObject* GO) { return new PortalCrystalDalaranLower(GO); }

	void OnActivate(Player* pPlayer)
	{
		pPlayer->SafeTeleport(571, 2817, 5807.75f, 588.347f, 661.505f, 3.64765f);		 //Teleport to Dalaran Crystal (191230)
	}
};
...
public:
	...
	void OnActivate(Player* pPlayer)
	{
		if ( pPlayer->GetMapMgr()->iInstanceMode == MODE_HEROIC_5MEN )
...
void SetupGoHandlers(ScriptMgr * mgr)
{
	...
	mgr->register_gameobject_script(191229, &PortalCrystalDalaranTop::Create);
	mgr->register_gameobject_script(191230, &PortalCrystalDalaranLower::Create);
